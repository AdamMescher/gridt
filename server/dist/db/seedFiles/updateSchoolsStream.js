...
const updateSchoolStream = async collection => {
  const basePath = "./db/seedData/2017-18-crdc-data-corrected-publication 2/2017-18 Public-Use Files/Data/SCH/EDFacts/CSV/ID 74 SCH - Race by Sex by Disability plus LEP";
  ...
  const filepaths = [plusAut, plusDb, plusDd, plusEmn, plusHi, plusMd, plusMr, plusOhi, plusOi, plusSld, plusSli, plusTbi, plusVi];
  var promises = filepaths.map(async file => new Promise((resolve, reject) => {
    fs.createReadStream(filepaths[i]).pipe(csv.parse({
      headers: true
    })).on("error", err => console.error(err)).on("data", async school => {
      const stats = {
        am: {
          m: school.AM_M_7,
          f: school.AM_F_7
      ...
      const query = {
        comboKey: parseInt(school.NCESSCH.toString(), 10)
      };
      const update = {
        $set: {
          [school.DISABILITY_CATEGORY]: stats
        }
      };
      const options = {
        new: true
  ...
  Promise.all(promises).then(res => console.log("SUCCESS")).catch(error => console.error(error));
};
